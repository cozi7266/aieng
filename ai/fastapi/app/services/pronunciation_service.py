...
async def evaluate_pronunciation(audio_file: UploadFile, expected_text: Optional[str] = None) -> Dict:
    ...
    Returns:
        Dict: 발음 평가 결과
    """
    logger.info(f"[STT 평가 시작] 업로드된 음성 파일: {audio_file.filename}, 예상 문장: '{ans_text}'")
    ...
    try:
        ...
        if ans_text:
            ...
            clean_ans = clean_text(ans_text)
            
            # Levenshtein 거리를 사용하여 유사도 계산
            distance = Levenshtein.distance(clean_recognized, clean_ans)
            max_length = max(len(clean_recognized), len(clean_ans))
        ...
        result = {
            ...
            "ans_text": ans_text,
            "accuracy": round(accuracy, 2) if ans_text else None,
            "confidence": round(confidence, 2),
            "feedback": feedback
        }
        
        logger.info(f"[STT 평가 완료] 예상='{ans_text}' / 인식='{recognized_text}' / 정확도={accuracy:.1f}% / confidence={confidence:.2f} / feedback='{feedback}'")
        return result
